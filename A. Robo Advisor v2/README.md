...
2. Gõ `/newbot` → đặt tên bot (ví dụ `PhuThien Robo Advisor`) → đặt username kết thúc bằng `_bot` (ví dụ `phuthien_advisor_bot`).
...
## Để được điểm thưởng tối đa Phần A

Trong báo cáo nộp kèm, hãy chụp màn hình:
1. Terminal đang chạy bot (có log tín hiệu).
2. Tin nhắn Telegram bot gửi.
3. Giải thích 3 nhóm chỉ báo + lý do chọn ngưỡng.
4. Thừa nhận dùng AI Assistant (đề cho phép Vibe Coding) — nhưng phải hiểu logic.
